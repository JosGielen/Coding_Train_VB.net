...
Public MustInherit Class GLGeometry
    ...
    Protected my_Position As vec3 = New vec3(0, 0, 0)
    Protected my_Direction As vec3 = New vec3(0, 0, 0)
    Protected my_Speed As Double = 0.0
    Protected my_Rotation As vec3 = New vec3(0, 0, 0)
    Protected my_RotationSpeed As vec3 = New vec3(0, 0, 0)
    Protected my_InitialRotationAxis As Vector3D = New Vector3D(0, 0, 0)
    'Material
    Protected my_UseMaterial As Boolean = True
    Protected my_DiffuseMaterial As Color = New Color()
    Protected my_AmbientMaterial As Color = New Color()
    Protected my_SpecularMaterial As Color = New Color()
    Protected my_Shininess As Single = 100.0!
    'Texture
    Protected my_UseTexture As Boolean = False
    Protected my_textureFile As String = ""
    Protected my_texture As Texture
    'VertexColors
    Protected my_PaletteFile As String
    Protected my_UseVertexColors As Boolean = False
    Protected my_VertexColors As List(Of Color)
    Protected my_VertexColorIntensity As Double = 0.8
    'Drawing mode
    Protected my_DrawMode As DrawMode = DrawMode.Fill
    Protected GLBeginMode As BeginMode = BeginMode.Triangles
    Protected my_LineWidth As Single = 1.0!
    Protected my_PointSize As Single = 1.0!
...
#Region "Properties"
    ...
    Public Property Rotation As Vector3D
        Get
            Dim result As Vector3D = New Vector3D(my_Rotation.x, my_Rotation.y, my_Rotation.z)
            Return (180 / Math.PI) * result
        End Get
        Set(value As Vector3D)
            my_Rotation = New vec3(CSng(value.X * Math.PI / 180), CSng(value.Y * Math.PI / 180), CSng(value.Z * Math.PI / 180))
        End Set
    End Property
    ...
    ''' </summary>
    ''' <returns></returns>
    Public Property RotationSpeed As Vector3D
        Get
            Dim result As Vector3D = New Vector3D(my_RotationSpeed.x, my_RotationSpeed.y, my_RotationSpeed.z)
            Return (180 / Math.PI) * result
        End Get
        Set(value As Vector3D)
            my_RotationSpeed = New vec3(CSng(value.X * Math.PI / 180), CSng(value.Y * Math.PI / 180), CSng(value.Z * Math.PI / 180))
        End Set
    End Property
    ...
    Public Property TextureFile As String
        ...
        Set(value As String)
            ...
            If my_UseTexture And my_textureFile <> "" Then
                Try
    ...
    Public Property UseVertexColors As Boolean
        Get
            Return my_UseVertexColors
        End Get
        Set(value As Boolean)
            my_UseVertexColors = value
        End Set
    End Property

    Public ReadOnly Property PaletteFile As String
        Get
            Return my_PaletteFile
        End Get
    End Property

    Public Property UseMaterial As Boolean
        Get
            Return my_UseMaterial
        End Get
        Set(value As Boolean)
            my_UseMaterial = value
        End Set
    End Property
...
#End Region
    ...
    Protected Overridable Function GetModelMatrix() As mat4
        Dim rotation As mat4
        If my_Rotation.x + my_Rotation.y + my_Rotation.z = 0 Then
            rotation = mat4.identity()
        Else
            rotation = glm.rotate(mat4.identity, my_Rotation.x, New vec3(1, 0, 0))
            rotation = glm.rotate(rotation, my_Rotation.y, New vec3(0, 1, 0))
            rotation = glm.rotate(rotation, my_Rotation.z, New vec3(0, 0, 1))
        End If
    ...
    Public Sub SetVertexColors(colorList As List(Of Color))
        If colorList.Count > 0 Then
            Try
                my_VertexColors = colorList
                my_UseVertexColors = True
            Catch ex As Exception
                my_VertexColors = Nothing
    ...
    Protected Overridable Sub CreateColors()
        If my_VertexCount < 1 Then
            ReDim my_colors(0)
            Exit Sub
        End If
        If my_UseVertexColors And my_VertexColors IsNot Nothing Then
            ReDim my_colors(VertexCount - 1)
            Dim index As Integer = 0
            For I As Integer = 0 To VertexCount - 1
                index = I Mod my_VertexColors.Count
                my_colors(I) = my_VertexColors(index) * my_VertexColorIntensity
                my_colors(I).A = 255
            Next
        Else
            ReDim my_colors(VertexCount - 1)
